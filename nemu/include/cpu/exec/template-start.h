...
#define MEM_R(addr) swaddr_read(addr, DATA_BYTE, ((addr<=cpu.ebp)&&(addr>=cpu.esp))? R_SS : R_DS)
#define MEM_W(addr, data) swaddr_write(addr, DATA_BYTE, data, ((addr<=cpu.ebp)&&(addr>=cpu.esp))? R_SS : R_DS)
